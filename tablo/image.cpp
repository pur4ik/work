...
static uint8_t redTable[32];
static uint8_t greenTable[64];
static uint8_t blueTable[32];
...
void Image::drawLayer(Layer& layer) {
    for (int y = 0; y < layer.height; ++y) {
        if (layer.fixHeight != -1 && y >= layer.fixHeight) continue;

        int finalY = layer.fromY + y;
        if (finalY < 0 || finalY >= height_image) continue;

        for (int x = 0; x < layer.width; ++x) {
            if (layer.fixWidth != -1 && x >= layer.fixWidth) continue;

            int finalX = layer.fromX + x;
            if (finalX < 0 || finalX >= width_image) continue;

            int layerIndex = (y * layer.width + x) * 2;
            uint16_t pixel = (layer.buffer[layerIndex + 1] << 8) | layer.buffer[layerIndex];

            uint8_t r = redTable[(pixel >> 11) & 0x1F];
            uint8_t g = greenTable[(pixel >> 5) & 0x3F];
            uint8_t b = blueTable[pixel & 0x1F];

            setPixel(finalX, finalY, r, g, b);
        }
    }

    // не требуется обновления координат
    if (!(layer.fromX != layer.toX || layer.fromY != layer.toY)) { return; }  

    // Обновляем позицию слоя
    if (layer.fromX != layer.toX) {
        layer.fromX += (layer.fromX < layer.toX) ? 1 : -1;

        if (layer.fromX == layer.toX) {
            layer.fromX = layer.initialFromX;
        }
    }

    if (layer.fromY != layer.toY) {
        layer.fromY += (layer.fromY < layer.toY) ? 1 : -1;
        
        if (layer.fromY == layer.toY) {
            layer.fromY = layer.initialFromY;
        }
    }
}
