...
int Bp::init() {
    ...
    std::cout << "ответ: " << responce << "\n";
    if (responce.length() != 0) {
        std::this_thread::sleep_for(std::chrono::milliseconds(20));
        uart.uart_write("so0\r\n");
        std::this_thread::sleep_for(std::chrono::milliseconds(20));
        uart.uart_write("su01100\r\n");
        std::this_thread::sleep_for(std::chrono::milliseconds(20));
        uart.uart_write("sj02500\r\n");
        std::this_thread::sleep_for(std::chrono::milliseconds(20));
...
int Bp::set_u_kp(float u_value) {
    std::string u_command;
    float mm_u;
    float bp_u;
    float div = 10.0;
    float test = u_value;
    while (abs(akip.read_u() - test) > 0.1) {
        // if (u_value >= 170.0){
        //     std::cout << "ПРЕВЫШЕНО ЗНАЧЕНИЕ" << "\n";
        //     continue;
        // }
        u_command = util.convert_float(u_value, 'u');
        uart.uart_write(u_command.c_str());
